...
async function handleUpdate(update: any) {
  ...
  const cfgRes = await supabaseAdmin.from("bot_config").select("*").eq("id", 1).single();
  ...
  const isAdmin = (cfg.admin_telegram_ids ?? []).map(Number).includes(Number(fromId));
...
async function advanceUpload(
  ...
  if (!stored) {
    await tg("sendMessage", { chat_id: chatId, text: "❌ Failed to copy to storage. Retry." });
    return true;
  }
