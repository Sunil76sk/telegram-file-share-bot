...
async def newpost_command(client: Client, message: Message):
    ...
    print(f"[POST BUILDER] /newpost from user={user_id}")
    ...
    async with user_locks[user_id]:
        ...
        if draft and draft.get("state") not in ("idle", "", None):
            buttons = [
...
async def _show_channel_selection(target, channels: list):
    ...
    buttons.append([InlineKeyboardButton("➕ Add Channel", callback_data="pb_add_channel")])
...
async def select_channel_callback(client: Client, callback_query: CallbackQuery):
    ...
    print(f"[POST BUILDER] Channel selected: user={user_id}, channel={channel_id_str}")

    try:
        channel_id_int = int(channel_id_str)
    except ValueError:
        channel_id_int = channel_id_str
    ...
    print(f"[POST BUILDER] Draft created: user={user_id}, state=awaiting_media")
...
async def select_type_callback(client: Client, callback_query: CallbackQuery):
    ...
    print(f"[POST BUILDER] Type selected: user={user_id}, type={post_type}")
    ...
    if post_type == "text":
        ...
        await database.save_post_draft(user_id, draft)
        print(f"[POST BUILDER] State changed: user={user_id}, state=awaiting_caption")
        await _prompt_caption_input(callback_query.message, draft)
    elif post_type in ("photo", "video", "document"):
        draft["state"] = "awaiting_media"
        await database.save_post_draft(user_id, draft)
        print(f"[POST BUILDER] State changed: user={user_id}, state=awaiting_media")
...
async def select_ratio_callback(client: Client, callback_query: CallbackQuery):
    ...
    print(f"[POST BUILDER] Ratio selected: user={user_id}, ratio={ratio}")
...
async def select_style_callback(client: Client, callback_query: CallbackQuery):
    ...
    print(f"[POST BUILDER] Style selected: user={user_id}, style={style}")
    ...
    if poster.get("file_id") and poster.get("type") == "photo":
        ...
        try:
            ...
            print(f"[POST BUILDER] Downloading image: file_id={file_id}")
            image_bytes = await client.download_media(file_id, in_memory=True)
            print(f"[POST BUILDER] Image downloaded: {len(image_bytes)} bytes")
            processed_bytes = await image_converter.fit_image(
                image_bytes, draft["image_ratio"], style
            )
            print(f"[POST BUILDER] Image processed: {len(processed_bytes)} bytes")
            ...
            print(f"[POST BUILDER] Image processing FAILED: {e}")
            draft["image_style"] = style
            draft["state"] = "awaiting_caption"
            await database.save_post_draft(user_id, draft)
            await _prompt_caption_input(callback_query.message, draft)
...
@app.on_message(
    ...
    & ~filters.command(
        [
            "start", "batch", "done", "cancel", "newpost", "help",
            "settings", "premium", "referral", "upload", "stats",
            "broadcast", "shorteners", "ads", "adstats", "traffic",
            "analytics", "advertise", "channel_stats", "searchmovie",
            "diag", "diagnose", "store", "mychannels", "channelsettings",
            "addchannel", "delchannel", "schedule", "reposts",
        ]
    )
    & ~banned_filter,
    group=3,
...
async def post_builder_text_handler(client: Client, message: Message):
    ...
    print(f"[POST BUILDER TEXT] user={user_id}, state={state}, text={text[:30]}")
    ...
    if state == "awaiting_caption":
        ...
        button = {"text": btn_text, "url": text, "type": btn_type, "id": uuid.uuid4().hex[:8]}
        ...
        status_msg = await message.reply_text(f"🔍 Searching TMDB for '**{text}**'...")
...
async def _handle_schedule_time_input(client: Client, message: Message, user_id: int, draft: dict, text: str):
    try:
        ...
        if scheduled_time <= datetime.datetime.now(datetime.timezone.utc):
            await message.reply_text("❌ **Invalid Time!** The scheduled time must be in the future.")
            return
...
async def _show_post_builder_menu(client, target, user_id: int, draft: dict):
    await _show_button_builder_menu(target, draft)
...
async def btn_type_callback(client: Client, callback_query: CallbackQuery):
    ...
    type_labels = {
        "url": "Website URL", "deep_link": "Deep Link (/start)",
        "channel": "Telegram Channel", "product": "Creator Store",
        "payment_upi": "UPI Payment", "payment_stars": "Telegram Stars",
    }
...
async def select_layout_callback(client: Client, callback_query: CallbackQuery):
    ...
    for layout_key, layout_label in LAYOUTS.items():
        ...
        buttons.append([
            InlineKeyboardButton(f"{marker}{layout_label}", callback_data=f"pb_layout_{layout_key}")
        ])
...
async def _publish_post(
    client: Client, user_id: int, draft: dict,
    ...
    keyboard = _build_post_keyboard(custom_buttons, download_files, layout_type)
    print(f"[POST BUILDER] Publishing: user={user_id}, channel={channel_id}, type={media_type}")
    ...
    try:
        ...
        if poster.get("file_id") and poster.get("type") == "photo":
            ...
            sent_message = await client.send_message(
                channel_id, caption or "📝 New post",
                reply_markup=keyboard if keyboard.inline_keyboard else None,
            )
        ...
        if sent_message:
            ...
            print(f"[POST BUILDER] Published: message_id={sent_message.id}")
            success_text = (
                f"✅ **Post Published Successfully!**\n\n"
                f"📢 **Channel:** {draft.get('channel_title', 'Unknown')}\n"
                f"🆔 **Message ID:** `{sent_message.id}`\n"
            )
            buttons = [[InlineKeyboardButton("📝 Create New Post", callback_data="pb_new_post")]]
            await client.send_message(user_id, success_text, reply_markup=InlineKeyboardMarkup(buttons))

    except Exception as e:
        error_text = str(e).lower()
        print(f"[POST BUILDER] Publish FAILED: {e}")
...
# ═══════════════════════════════════════════════════════════════════════
#  MEDIA HANDLER (group=4, HIGHER PRIORITY than upload.py group=1)
#  This runs AFTER upload.py group=1 which returns early for builder states.
#  We catch the media here and route it to the post builder.
# ═══════════════════════════════════════════════════════════════════════
...
async def post_builder_media_handler(client: Client, message: Message):
    ...
    print(f"[POST BUILDER MEDIA] Captured media: user={user_id}, state={state}")
    ...
    print(f"[POST BUILDER MEDIA] file_id={file_id}, media_type={media_type}")
    ...
    if media_type == "photo":
        await database.save_post_draft(user_id, draft)
        print(f"[POST BUILDER MEDIA] Saved to draft: user={user_id}, showing ratio selection")
        await _show_ratio_selection(message, draft)
    else:
        draft["state"] = "awaiting_caption"
        await database.save_post_draft(user_id, draft)
        print(f"[POST BUILDER MEDIA] State changed: user={user_id}, state=awaiting_caption")
