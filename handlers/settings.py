...
async def re_shorten_tutorial_url():
    ...
    try:
        ...
        logger.info(
            f"Re-shortened tutorial URL successfully. Shortened: {short_url or long_url}"
        )
...
async def show_settings_menu(client: Client, msg: Message, user_id: int):
    ...
    kb_rows = [
        ...
        [
            InlineKeyboardButton(
                f"🔔 Notifications: {'✅' if notif_prefs else '❌'}",
                callback_data="settings_notif",
            )
        ],
        [
            InlineKeyboardButton(
                "🎫 Premium / Subscription", callback_data="premium_menu_home"
            )
        ],
        [
            InlineKeyboardButton(
                "👥 Referral Program", callback_data="settings_referral"
            )
        ],
        [InlineKeyboardButton("📋 My Channels", callback_data="settings_channels")],
        [
            InlineKeyboardButton(
                "🔄 Auto Delete Settings", callback_data="settings_autodel"
            )
        ],
    ]
    ...
    if await database.is_admin(user_id, client):
        kb_rows.append(
            [
                InlineKeyboardButton(
                    "🛠 Global Admin Settings", callback_data="settings_admin_menu"
                )
            ]
        )
...
async def show_admin_settings_menu(
    client: Client, msg: Message | CallbackQuery, user_id: int
):
    settings = await database.get_settings()
    ...
    kb = InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton(
                    "🔑 Update TMDB Key", callback_data="admin_set_tmdb_key"
                ),
                InlineKeyboardButton(
                    "🌐 Default Lang", callback_data="admin_set_tmdb_lang"
                ),
            ],
            [
                InlineKeyboardButton(
                    "📹 Tutorial URL", callback_data="admin_set_tutorial_url"
                ),
                InlineKeyboardButton(
                    f"🎥 Append: {append_status}",
                    callback_data="admin_toggle_tutorial_append",
                ),
            ],
            [
                InlineKeyboardButton(
                    "🔙 Back to User Settings", callback_data="settings_menu"
                )
            ],
        ]
    )
...
@app.on_callback_query(
    filters.regex(
        r"^settings_(lang|tz|notif|referral|channels|autodel|menu|admin_menu)$"
    )
)
async def settings_callback_handler(client: Client, callback_query: CallbackQuery):
    ...
    if action == "lang":
        ...
        for code, name in langs.items():
            kb.append(
                [InlineKeyboardButton(f"{name}", callback_data=f"setlang_{code}")]
            )
...
@app.on_callback_query(
    filters.regex(
        r"^admin_(set_tmdb_key|set_tmdb_lang|set_tutorial_url|toggle_tutorial_append)$"
    )
)
async def admin_settings_callback_handler(
    ...
    if action == "set_tmdb_key":
        await database.users_col.update_one(
            {"_id": user_id}, {"$set": {"state": "awaiting_tmdb_api_key"}}
        )
        await callback_query.message.edit_text(
            "🔑 **Update TMDB API Key**\n\n"
        ...
        await database.users_col.update_one(
            {"_id": user_id}, {"$set": {"state": "awaiting_tmdb_default_language"}}
        )
        await callback_query.message.edit_text(
        ...
        await database.users_col.update_one(
            {"_id": user_id}, {"$set": {"state": "awaiting_tutorial_video_url"}}
        )
        await callback_query.message.edit_text(
...
async def set_lang_callback_handler(client: Client, callback_query: CallbackQuery):
    ...
    await callback_query.answer(
        f"Language set to {lang_code.upper()}!", show_alert=True
    )
...
async def settings_input_handler(client: Client, message: Message):
    ...
    if not state or state not in [
        "awaiting_tmdb_api_key",
        "awaiting_tmdb_default_language",
        "awaiting_tutorial_video_url",
    ]:
        return
    ...
    if state == "awaiting_tmdb_api_key":
        ...
        if not text or len(text) > 5:
            await message.reply_text(
                "❌ Invalid language code. E.g. en, es, hi. Try again:"
            )
            message.stop_propagation()
            return
        ...
        await database.users_col.update_one({"_id": user_id}, {"$unset": {"state": ""}})
        await message.reply_text(
            f"✅ TMDB Default Language set to `{text.lower()}` successfully!"
        )
        await show_admin_settings_menu(client, message, user_id)
        message.stop_propagation()
        return
        ...
        if not text.startswith("http"):
            await message.reply_text(
                "❌ Invalid URL. Must start with http:// or https://. Try again:"
            )
            message.stop_propagation()
            return
        ...
        await database.update_settings(
            {"tutorial_video_url": text, "tutorial_shortened_url": short_url or text}
        )
        await database.users_col.update_one({"_id": user_id}, {"$unset": {"state": ""}})
        await message.reply_text(
            "✅ Tutorial Video URL updated and shortened successfully!"
        )
        await show_admin_settings_menu(client, message, user_id)
        message.stop_propagation()
        return
