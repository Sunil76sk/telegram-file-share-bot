...
async def deliver_product_files(client: Client, chat_id: int, purchase: dict, product: dict):
    """Deliver product files to user instantly and track downloads."""
    files = product.get("files", [])
    if not files:
        await client.send_message(chat_id, "❌ No files found for this product. Please contact support.")
        return
    ...
    for index, file_obj in enumerate(files):
        ...
        try:
            ...
            logger.error(f"Failed to deliver product file index {index} to user {chat_id}: {e}")
            await client.send_message(
                chat_id,
...
@app.on_message(filters.command(["marketplace", "market", "shop_products"]) & filters.private & ~banned_filter)
async def marketplace_command_handler(client: Client, message: Message):
    ...
    for cat in categories:
        buttons.append([InlineKeyboardButton(f"{cat['icon']} {cat['name']}", callback_data=f"mkt_cat_{cat['slug']}")])
...
async def mkt_category_callback(client: Client, callback_query: CallbackQuery):
    ...
    if not products:
        ...
        keyboard.append([InlineKeyboardButton("➕ Sell a Product", callback_data=f"mkt_add_cat_{category_slug}")])
    else:
        ...
        for p in products:
            ...
            keyboard.append([InlineKeyboardButton(f"📄 {p['name']} {price_label}", callback_data=f"mkt_prod_{p['_id']}")])

    keyboard.append([InlineKeyboardButton("🔙 Back to Categories", callback_data="mkt_menu")])
    await callback_query.answer()
...
async def mkt_product_detail_callback(client: Client, callback_query: CallbackQuery):
    ...
    await callback_query.answer()
    await show_product_card(client, callback_query.message.chat.id, product, user_id, edit_message_id=callback_query.message.id)
...
async def mkt_download_callback(client: Client, callback_query: CallbackQuery):
    ...
    if not (is_owner or is_bot_admin or has_purchased or is_free):
        await callback_query.answer("⛔️ Access denied. Please purchase this product first.", show_alert=True)
        return
...
@app.on_callback_query(filters.regex(r"^mkt_buy_(stars|upi)_([a-fA-F0-9]{24})$") & ~banned_filter)
async def mkt_buy_callback(client: Client, callback_query: CallbackQuery):
    ...
    if method == "stars":
        ...
        try:
            ...
            logger.error(f"Failed to send stars invoice for marketplace product {prod_id}: {e}")
            await client.send_message(
                chat_id=user_id,
...
async def mkt_seller_dashboard_callback(client: Client, callback_query: CallbackQuery):
    ...
    for p in my_products:
        ...
        total_revenue_upi += sum(pur.get("amount_paid", 0) for pur in purchases if pur.get("status") == "completed")
    ...
    buttons = [
        ...
        [InlineKeyboardButton("📦 My Products", callback_data="mkt_my_products_list")] if total_products > 0 else [],
...
async def mkt_my_products_list_callback(client: Client, callback_query: CallbackQuery):
    ...
    for p in my_products:
        ...
        keyboard.append([InlineKeyboardButton(f"{status_symbol} {p['name']}", callback_data=f"mkt_prod_{p['_id']}")])
        
    keyboard.append([InlineKeyboardButton("🔙 Back to Dashboard", callback_data="mkt_seller_dashboard")])
    await callback_query.answer()
    await callback_query.message.edit_text(msg, reply_markup=InlineKeyboardMarkup(keyboard))


@app.on_callback_query(filters.regex(r"^mkt_pstats_([a-fA-F0-9]{24})$") & ~banned_filter)
async def mkt_product_stats_callback(client: Client, callback_query: CallbackQuery):
    ...
    keyboard = [
        [InlineKeyboardButton("🔙 Back to Product Details", callback_data=f"mkt_prod_{prod_id}")]
    ]
    await callback_query.answer()
    await callback_query.message.edit_text(msg, reply_markup=InlineKeyboardMarkup(keyboard))
...
async def mkt_my_purchases_callback(client: Client, callback_query: CallbackQuery):
    ...
    for p in purchases:
        if p.get("status") == "completed" and p["product_id"] not in product_ids_seen:
            ...
            if product:
                keyboard.append([InlineKeyboardButton(f"🎁 {product['name']}", callback_data=f"mkt_prod_{product['_id']}")])

    if not keyboard:
        msg = "🛍 **My Purchased Products:**\n\nYou haven't purchased any products yet."

    keyboard.append([InlineKeyboardButton("🔙 Back to Marketplace", callback_data="mkt_menu")])
    await callback_query.answer()
    await callback_query.message.edit_text(msg, reply_markup=InlineKeyboardMarkup(keyboard))
...
@app.on_message(filters.command(["add_product", "sell"]) & filters.private & ~banned_filter)
async def add_product_command_handler(client: Client, message: Message):
    ...
    buttons = []
    for cat in categories:
        buttons.append([InlineKeyboardButton(f"{cat['icon']} {cat['name']}", callback_data=f"mkt_wizard_cat_{cat['slug']}")])
    buttons.append([InlineKeyboardButton("❌ Cancel", callback_data="mkt_wizard_cancel")])
...
@app.on_callback_query(filters.regex(r"^mkt_wizard_cat_([a-zA-Z0-9_]+)$") & ~banned_filter)
async def mkt_wizard_cat_callback(client: Client, callback_query: CallbackQuery):
    ...
    await callback_query.message.edit_text(
        ...
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("❌ Cancel", callback_data="mkt_wizard_cancel")]])
    )
...
async def mkt_wizard_cancel_callback(client: Client, callback_query: CallbackQuery):
    ...
    await database.users_col.update_one(
        {"_id": user_id},
        {"$unset": {"state": "", "marketplace_draft": ""}}
    )
    await callback_query.answer("Wizard cancelled.")
    await callback_query.message.edit_text("❌ Product creation cancelled.")


async def handle_marketplace_state(client: Client, message: Message, user_id: int, state: str, user_doc: dict):
    ...
    if state == "market_awaiting_name":
        if not text:
            await message.reply_text("❌ Product name cannot be empty. Please send a valid name:")
            return
        ...
        await database.users_col.update_one(
            {"_id": user_id},
            {"$set": {"state": "market_awaiting_price_stars", "marketplace_draft": draft}}
        )
        await message.reply_text(
        ...
        await database.users_col.update_one(
            {"_id": user_id},
            {"$set": {"state": "market_awaiting_price_upi", "marketplace_draft": draft}}
        )
        await message.reply_text(
        ...
        if draft.get("price_stars", 0) == 0 and price_upi == 0.0:
            await message.reply_text("❌ Both Stars price and UPI price cannot be 0. Please enter a valid UPI price higher than 0.0:")
            return
        ...
        await message.reply_text(
            ...
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("✅ Done Uploading", callback_data="mkt_wizard_done")]])
        )
        ...
        if not file_id:
            await message.reply_text("❌ Please send a valid file (Document, Video, Audio, Photo, etc.).")
            return
...
async def mkt_wizard_done_callback(client: Client, callback_query: CallbackQuery):
    ...
    if not user_doc or user_doc.get("state") != "market_awaiting_files":
        await callback_query.answer("⚠️ No active creation wizard session.", show_alert=True)
        return

    draft = user_doc.get("marketplace_draft", {})
    files = draft.get("files", [])
    if not files:
        await callback_query.answer("❌ You must upload at least one file before finishing!", show_alert=True)
        return
...
async def show_product_card(client: Client, chat_id: int, product: dict, user_id: int, edit_message_id: int = None):
    """Render and display product details card to the user."""
    # Determine product type display name and icon
    type_name = database.PRODUCT_TYPE_NAMES.get(product["product_type"], product["product_type"].replace("_", " ").title())
    type_icon = database.PRODUCT_TYPE_ICONS.get(product["product_type"], "📁")

    creator_name = "Creator"
    creator = await database.get_user(product["owner_id"])
    if creator:
        creator_name = f"@{creator.get('username')}" if creator.get("username") else creator.get("first_name", "Creator")
    ...
    if is_owner or is_bot_admin or has_purchased:
        ...
        buttons.append([InlineKeyboardButton("📥 Download Product Files", callback_data=f"mkt_dl_{product['_id']}")])
        if is_owner or is_bot_admin:
            buttons.append([
                InlineKeyboardButton("📊 Stats", callback_data=f"mkt_pstats_{product['_id']}"),
                InlineKeyboardButton("🗑 Delete Product", callback_data=f"mkt_del_{product['_id']}")
            ])
    else:
        ...
        if stars_price == 0 and upi_price == 0.0:
            ...
            if stars_price > 0:
                ...
                buy_row.append(InlineKeyboardButton(f"⭐️ Buy ({stars_price} Stars)", callback_data=f"mkt_buy_stars_{product['_id']}"))
            if upi_price > 0.0:
                msg += f"• **UPI Transfer:** ₹{upi_price} INR\n"
                buy_row.append(InlineKeyboardButton(f"💸 Buy (₹{upi_price} UPI)", callback_data=f"mkt_buy_upi_{product['_id']}"))
            
            buttons.append(buy_row)

    cat_slug = product["product_type"]
    buttons.append([InlineKeyboardButton("🔙 Back to List", callback_data=f"mkt_cat_{cat_slug}")])
