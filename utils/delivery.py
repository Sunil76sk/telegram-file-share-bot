        ...
        if failures == len(files):
            ...
            if not is_premium:
                try:
                    ...
                    active_pinned_ads = [
                        a for a in active_pinned_ads if a.get("status") == "active"
                    ]
                    if active_pinned_ads:
                        ...
                        reply_markup = InlineKeyboardMarkup(
                            [
                                [
                                    InlineKeyboardButton(
                                        ad.get("button_text") or "Visit Sponsor 🌐",
                                        url=click_url,
                                    )
                                ]
                            ]
                        ...
                        ad_msg = await client.send_message(
                            chat_id, text=ad_text, reply_markup=reply_markup
                        )
                        if ad_msg:
                    ...
                    logger.error(
                        f"Failed to inject pinned ad for user {chat_id}: {ad_err}"
                    )
