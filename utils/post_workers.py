                ...
                try:
                    ...
                    if media_type == "photo" and poster_url:
                        ...
                        if os.path.exists(full_image_path):
                            ...
                            logger.error(f"Scheduled post image not found: {full_image_path}. Sending text fallback.")
                    ...
                    if sent_msg:
                        ...
                        await database.increment_channel_stat(post["channel_id"], "posts", 1)
                    ...
                    if retry_count < 3:
                        await database.mark_post_retry(post_id_str, retry_count, str(ex))
                    else:
                        await database.mark_post_failed(post_id_str, f"Retries exhausted. Last error: {ex}")
                ...
                try:
                    ...
                    if job.get("delete_old", True) and job.get("last_post_id"):
                        try:
                            ...
                            logger.info(f"Deleted old repost message {job['last_post_id']} in channel {job['channel_id']}.")
                        except Exception as del_ex:
                            logger.warning(f"Failed to delete old message {job['last_post_id']} in channel {job['channel_id']}: {del_ex}")
                    ...
                    if media_type == "photo" and poster_url:
                        ...
                        if os.path.exists(full_image_path):
                            ...
                            logger.error(f"Repost image not found: {full_image_path}. Sending text fallback.")
                            sent_msg = await app.send_message(
                                chat_id=job["channel_id"],
                                text=caption,
                                reply_markup=reply_markup
                    ...
                    if sent_msg:
                        ...
                        await database.CHANNEL_POST_HISTORY_COL.insert_one(history_doc)
                        await database.increment_channel_stat(job["channel_id"], "posts", 1)
                        
                        # 7. Update repost job configuration for next execution
                        interval_minutes = job.get("repost_interval", 60)
                        next_post_at = datetime.datetime.now(datetime.timezone.utc) + datetime.timedelta(minutes=interval_minutes)
                        await database.update_repost_job_run(job_id_str, sent_msg.id, next_post_at)
                        logger.info(f"Successfully processed repost job {job_id_str}. Next run scheduled at {next_post_at}.")
                        
                except Exception as ex:
                    logger.error(f"Error executing repost job {job_id_str}: {ex}")
                    retry_count = job.get("retry_count", 0) + 1
                    if retry_count < 3:
                        await database.mark_repost_job_retry(job_id_str, retry_count, str(ex))
                    else:
                        await database.mark_repost_job_failed(job_id_str, f"Retries exhausted. Last error: {ex}")
